...
local AF_TSub_014 = [[ Auto Farm Mars ]]
...
Section1:NewToggle(AF_T_014, AF_TSub_014, function(FM_0014)
    getgenv().Farm14 = FM_0014

    while wait() do
        if getgenv().Farm14 == true then
            pcall(function()
                for i,v in pairs(game.Workspace.Mobs["14"]:GetChildren()) do
                        if v.Humanoid.Health > 0 and game.Players.LocalPlayer.Character.Humanoid.Health > 0 then
                            repeat
                                wait()
                                game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = v.HumanoidRootPart.CFrame * CFrame.new(0,0,2)
                            until v.Humanoid.Health <= 0 or getgenv().Farm14 == false
                    end
                end
            end)
        end
...
Section2:NewToggle("Open : Space Egg 2 ", "Auto Open Space Egg 2 ", function(M_egg00101010)
    if M_egg00101010 then
        _G.DC_Aegg00101010 = true
while _G.DC_Aegg00101010 do wait(DNA_Wait_time1)
    pcall(function()
local args101010 = {
    [1] = "Space Egg 2",
    [2] = "Hatch3"
...
game:GetService("ReplicatedStorage").Remotes.Gameplay.RequestPetPurchase:InvokeServer(unpack(args101010))
    end)
end
    else
        _G.DC_Aegg0010 = false
...
Section2:NewToggle("Open : Mars Egg 2 ", "Auto Open Mars Egg 2 ", function(M_egg001010101)
    if M_egg001010101 then
        _G.DC_Aegg001010101 = true
while _G.DC_Aegg001010101 do wait(DNA_Wait_time1)
    pcall(function()
local egg0001 = {
    [1] = "Mars Egg 2",
    [2] = "Hatch3"
...
game:GetService("ReplicatedStorage").Remotes.Gameplay.RequestPetPurchase:InvokeServer(unpack(egg0001))
    end)
end
    else
        _G.DC_Aegg001010101 = false
    end
end)
...
local TP_IL14 = CFrame.new(2589, 20+40.37624740600586, 8514)
...
local TP_T_014 = [[ Teleport To : Mars ]]
...
local TP_TSub_014 = [[ Teleport Mars ]]
...
Section3:NewButton(TP_T_014, TP_TSub_014, function()
    pcall(function()
    local TP_1 = TweenService:Create(TS_1, TS_2, 
    {CFrame = TP_IL14}):Play()
    end)
